...
import { DollarSign, CreditCard, Banknote, HelpCircle, ShieldCheck, Receipt } from "lucide-react"
...
const CONFIG = {
  ...
  INSURANCE: {
    icon: ShieldCheck,
    title: "Nộp Bảo hiểm",
    description: "Nộp toàn bộ BH (NLĐ 10.5% + DN 23.5%) cho cơ quan BHXH. Hạch toán Nợ 338 / Có 111 hoặc 112.",
    color: "blue",
  ...
  TAX: {
    icon: Receipt,
    title: "Nộp Thuế TNCN",
    description: "Nộp thuế Thu nhập cá nhân cho cơ quan Thuế. Hạch toán Nợ 3335 / Có 111 hoặc 112.",
    color: "amber",
...
export default function PaymentDialog({ onPay, disabled, paymentType }: PaymentDialogProps) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md p-6 bg-white rounded-2xl shadow-2xl border-none">
        <div className="text-center space-y-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-2 ${cfg.iconBg}`}>
            <Icon className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">{cfg.title}</h2>
