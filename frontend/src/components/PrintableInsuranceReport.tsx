        ...
        <table className="report-table">
          <thead>
            <tr>
              ...
              <th rowSpan={2} style={{ width: '110px' }}>THU NHẬP CHỊU</th>
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={item.employeeId}>
                ...
                <td className="text-right" style={{ fontWeight: 'bold' }}>{formatCurrency(item.totalEE)}</td>
              </tr>
            ))}
            {data.length === 0 && (
              <tr>
                <td colSpan={11} className="text-center" style={{ fontStyle: 'italic', padding: '20px' }}>
          ...
          {data.length > 0 && (
            <tfoot>
              <tr style={{ fontWeight: 'bold' }}>
                ...
                <td colSpan={3}></td>
                <td className="text-right">{formatCurrency(data.reduce((sum, i) => sum + i.bhxhEE, 0))}</td>
                <td className="text-right">{formatCurrency(data.reduce((sum, i) => sum + i.bhytEE, 0))}</td>
                <td className="text-right">{formatCurrency(data.reduce((sum, i) => sum + i.bhtnEE, 0))}</td>
                <td className="text-right">{formatCurrency(data.reduce((sum, i) => sum + i.totalEE, 0))}</td>
              </tr>
            </tfoot>
          )}
        </table>
