        ...
        <table className="report-table">
          <thead>
            <tr>
              ...
              <th rowSpan={2} style={{ width: '200px' }}>HỌ VÀ TÊN</th>
              <th rowSpan={2} style={{ width: '100px' }}>MÃ SỐ THUẾ</th>
              <th rowSpan={2} style={{ width: '120px' }}>TỔNG THU NHẬP</th>
              <th colSpan={4}>CÁC KHOẢN GIẢM TRỪ</th>
              <th rowSpan={2} style={{ width: '120px' }}>THU NHẬP TÍNH THUẾ</th>
              <th rowSpan={2} style={{ width: '110px' }}>THUẾ TNCN PHẢI NỘP</th>
            </tr>
            <tr>
          ...
          <tbody>
            {data.map((item, index) => {
              ...
              const totalDeduction = SELF_DEDUCTION + depTotal + item.totalInsurance;
              ...
              return (
                <tr key={item.employeeId}>
                  ...
                  <td>{item.fullName}</td>
                  <td className="text-center">{/* Trống nếu không có MST */}</td>
                  <td className="text-right">{formatCurrency(item.grossIncome)}</td>
                  <td className="text-right">{formatCurrency(SELF_DEDUCTION)}</td>
                  <td className="text-right">{formatCurrency(depTotal)}</td>
                  <td className="text-right">{formatCurrency(item.totalInsurance)}</td>
                  <td className="text-right" style={{ fontWeight: 'bold' }}>{formatCurrency(totalDeduction)}</td>
                  <td className="text-right">{formatCurrency(taxAssessedIncome)}</td>
                  <td className="text-right" style={{ fontWeight: 'bold' }}>{formatCurrency(item.taxAmount)}</td>
                </tr>
              );
            })}
            {data.length === 0 && (
              <tr>
                <td colSpan={10} className="text-center" style={{ fontStyle: 'italic', padding: '20px' }}>
          ...
          {data.length > 0 && (
            <tfoot>
              <tr style={{ fontWeight: 'bold' }}>
                ...
                <td></td>
                <td className="text-right">{formatCurrency(data.reduce((sum, i) => sum + (i.grossIncome - i.totalInsurance), 0))}</td>
