...
export default function AttendancePage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("cannot fetch params");
      }
      ...
      const resEmp = await axios.get(
        `/api/employees?page=0&size=2000&month=${month}&year=${year}`,
        auth,
      );
      const empList = resEmp.data.content;

      // Lấy dữ liệu công đã lưu (nếu có) - API này cũng cần phân trang theo nhân sự
      const resAtt = await axios.get(
        `/api/attendance/${month}/${year}?page=0&size=2000`,
        auth,
      );
      ...
      let suggestions: Record<
        string,
        { physicalDays: number; paidLeaveDays: number }
      > = {};
      try {
        const bulkRes = await axios.post(
          "/api/attendance/suggest-bulk",
          {
      ...
      setIsLocked(false);
      try {
        const resPayroll = await axios.get(
          `/api/payroll/${month}/${year}?page=0&size=1`,
          auth,
        );
        const payrolls = resPayroll.data.content;
        const isCalculationDone =
          payrolls.length > 0 &&
          payrolls.some((p: { status: string }) => p.status !== "REJECTED");
  ...
  const filteredAttendances = useMemo(() => {
    return attendances.filter((att) => {
      ...
      return (
        (att.employee.id || "").toLowerCase().includes(term) ||
  ...
  const handleUpdateLine = (empId: string, field: string, value: number) => {
    setAttendances((prev) =>
      prev.map((a) => (a.employee.id === empId ? { ...a, [field]: value } : a)),
    );
  };
  ...
  const handleSave = async () => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn lưu bảng chấm công này? Dữ liệu sẽ được dùng để tính lương.",
      )
    )
      return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 bg-muted/50 p-2 rounded-lg border">
          ...
          <Button
            ...
            className="gap-2 h-8 border-green-600 text-green-600 hover:bg-green-50"
          >
          ...
          <Button
            ...
            title={
              isLocked
                ? "Bảng lương đã được tính. Vui lòng từ chối bảng lương nếu muốn sửa lại công."
                : year * 12 + month >
                    new Date().getFullYear() * 12 + new Date().getMonth() + 1
                  ? "Không thể chấm công tháng tương lai"
                  : ""
            }
          >
      ...
      {/* ==============================================
            KHU VỰC VẼ BẢNG HIỂN THỊ DANH SÁCH CHẤM CÔNG
            (Hiển thị công làm việc và ngày phép, hỗ trợ nhập liệu trực tiếp)
            ============================================== */}
        ...
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="bg-primary text-primary-foreground">
            <tr>
              ...
              <th className="px-6 py-4 font-medium text-center bg-primary/90">
                Ngày công thực tế (ngày)
              </th>
              <th className="px-6 py-4 font-medium text-center bg-primary/80">
                Nghỉ hưởng lương (ngày)
              </th>
              <th className="px-6 py-4 font-medium text-center bg-primary/70">
                OT Thường (giờ)
              </th>
              <th className="px-6 py-4 font-medium text-center bg-primary/60">
                OT Cuối tuần (giờ)
              </th>
              <th className="px-6 py-4 font-medium text-center bg-primary/50">
                OT Lễ Tết (giờ)
              </th>
