...
export default function EmployeeList() {
  ...
  const [currentEmp, setCurrentEmp] = useState<Partial<Employee>>({
    ...
    contractSalary: 0,
    dependentCount: 0,
    positionCoefficient: 0.0,
    seniorityAllowance: 0.0,
  ...
  const validate = () => {
    ...
    if (
      currentEmp.phone &&
      !/^(0|\+84)\d{8,11}$/.test(currentEmp.phone.replace(/\s/g, ""))
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile && savedEmp) {
        ...
        await axios.post(
          `/api/employees/upload-contract/${savedEmp.id}`,
          formData,
          {
            headers: {
              ...auth.headers,
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        `Xác nhận nhân viên ${id} thôi việc? Hệ thống sẽ chuyển trạng thái sang "Đã nghỉ" để theo dõi.`,
      )
    )
      return;
    try {
  ...
  const handleDownloadContract = (id: string) => {
    ...
    window.open(
      `/api/employees/download-contract/${id}?access_token=${token}`,
      "_blank",
    );
  };
  ...
  const handleExportExcel = () => {
    ExportService.exportToExcel(employees, "Danh_sach_nhan_vien", "Nhân viên", {
      id: "Mã NV",
      fullName: "Họ tên",
      dob: "Ngày sinh",
      phone: "Số điện thoại",
    ...
    <div className="space-y-6">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  value={currentEmp.id}
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, id: e.target.value })
                  }
                  disabled={isEditing || viewOnly}
                  placeholder={viewOnly ? "" : "NV001"}
                  required
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, fullName: e.target.value })
                  }
                  disabled={viewOnly}
                  placeholder={viewOnly ? "" : "Nguyễn Văn A"}
                  required
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, dob: e.target.value })
                  }
                  disabled={viewOnly}
                  required
                />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, phone: e.target.value })
                  }
                  disabled={viewOnly}
                  placeholder={viewOnly ? "" : "09xxx"}
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, email: e.target.value })
                  }
                  disabled={viewOnly}
                  placeholder={viewOnly ? "" : "abc@company.com"}
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">
                ...
                <Input
                  value={currentEmp.hometown}
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, hometown: e.target.value })
                  }
                  disabled={viewOnly}
                  placeholder={viewOnly ? "" : "Hà Nội"}
                />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">
                  Lương hợp đồng (VNĐ) <span className="text-red-500">*</span>
                </label>
                <Input
                  type="text"
                  value={new Intl.NumberFormat("vi-VN").format(
                    currentEmp.contractSalary || 0,
                  )}
                  onChange={(e) => {
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCurrentEmp({
                      ...currentEmp,
                      seniorityAllowance: Number(e.target.value),
                    })
                  }
                  disabled={viewOnly}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, department: e.target.value })
                  }
                  disabled={viewOnly}
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setCurrentEmp({ ...currentEmp, gender: e.target.value })
                  }
                  disabled={viewOnly}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  value={currentEmp.employeeType}
                  onChange={(e) =>
                    setCurrentEmp({
                      ...currentEmp,
                      employeeType: e.target.value as Employee["employeeType"],
                    })
                  }
                  disabled={viewOnly}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              ...
              {currentEmp.status === "LEFT" && (
                <div className="space-y-2">
                  <label className="text-sm font-medium text-red-600 font-bold">
                    Ngày nghỉ việc
                  </label>
                  <Input
                    type="date"
                    value={currentEmp.resignationDate}
                    onChange={(e) =>
                      setCurrentEmp({
                        ...currentEmp,
            ...
            <div className="border-t pt-4 space-y-4">
              <h3 className="font-semibold flex items-center gap-2">
                <FileText className="w-4 h-4" /> Hợp đồng lao động (PDF)
              </h3>
              <div className="flex items-center gap-4">
                ...
                {currentEmp.contractFilePath && (
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => handleDownloadContract(currentEmp.id!)}
            ...
            {!viewOnly && (
              <div className="flex justify-end pt-4">
                <Button type="submit" className="px-10">
                  {isEditing ? "Lưu thay đổi" : "Lưu hồ sơ"}
                </Button>
              </div>
            )}
          </form>
      ...
      {!showForm && (
        <div className="space-y-4">
          ...
          {/* ==============================================
              KHU VỰC VẼ BẢNG HIỂN THỊ DANH SÁCH NHÂN SỰ
              (Liệt kê các nhân viên theo trang, tìm kiếm và lọc)
              ============================================== */}
          <div className="border border-slate-100 rounded-[2rem] bg-white/70 backdrop-blur-xl shadow-2xl shadow-slate-200/50 overflow-hidden">
            <table className="w-full text-sm text-left">
              <thead className="bg-primary text-primary-foreground">
                <tr>
                  <th className="px-6 py-5 font-black uppercase tracking-tighter">
                    Mã NV
                  </th>
                  <th className="px-6 py-5 font-black uppercase tracking-tighter">
                    Họ tên nhân viên
                  ...
                  <th className="px-6 py-5 font-black uppercase tracking-tighter text-center">
                    Phòng ban
                  </th>
                  <th className="px-6 py-5 font-black uppercase tracking-tighter text-center">
                    Trạng thái
              ...
              <tbody className="divide-y divide-slate-100">
                {pagedEmployees.map((employee) => (
                  ...
                  >
                    <td className="px-6 py-5 font-black text-slate-400 group-hover:text-primary transition-colors tabular-nums">
                      {employee.id}
                    </td>
                      ...
                      {employee.status === "LEFT" &&
                        employee.resignationDate && (
                          <div className="text-[10px] text-red-400 font-bold mt-1">
                            Nghỉ từ: {employee.resignationDate}
                          </div>
                        )}
                    </td>
                    <td className="px-6 py-5">
                ...
                {pagedEmployees.length === 0 && (
                  <tr>
                    <td
                      colSpan={6}
                      className="text-center py-20 text-slate-300 italic"
                    >
