...
export default function GeneralLedgerPage() {
  ...
  const fetchLedger = useCallback(async () => {
    ...
    try {
      ...
      // Cấu trúc mới: res.data = { pageResponse: { content, ... }, totalDebit, totalCredit }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center bg-white p-4 rounded-xl shadow-sm border gap-4">
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-3 bg-slate-50/80 p-1.5 rounded-lg border shadow-sm">
          ...
          <div className="flex items-center gap-2 px-2">
            <Input
              type="number"
              className="w-16 h-8 text-center bg-white font-medium shadow-sm transition-all focus:border-primary"
              value={month}
              onChange={(e) => {
                setPage(0);
            ...
            <Input
              type="number"
              className="w-20 h-8 text-center bg-white font-medium shadow-sm transition-all focus:border-primary"
              value={year}
              onChange={(e) => {
                setPage(0);
          ...
          <div className="flex items-center gap-2 px-1">
            <Button
              ...
              className="h-8 gap-1.5 border-green-600 text-green-600 hover:bg-green-50 shadow-sm bg-white hover:text-green-700"
            >
            ...
            <Button
              ...
              variant="outline"
              onClick={handlePrint}
              className="h-8 gap-1.5 border-blue-600 text-blue-600 hover:bg-blue-50 shadow-sm bg-white hover:text-blue-700"
            >
      ...
      <div className="border rounded-xl bg-card shadow-lg overflow-hidden">
        {/* ==============================================
                KHU VỰC VẼ BẢNG: SỔ CÁI TÀI KHOẢN KẾ TOÁN
                ============================================== */}
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-800 text-white border-b">
            <tr>
              ...
              <th className="px-4 py-3 font-bold" colSpan={2}>
                Chứng từ
              </th>
              <th className="px-4 py-3 font-bold" rowSpan={2}>
                Diễn giải
              </th>
              <th className="px-4 py-3 font-bold text-center" rowSpan={2}>
                TK Đối ứng
              </th>
              <th
                className="px-4 py-3 font-bold text-right border-l"
                colSpan={2}
              >
                Số phát sinh
              </th>
            ...
            <tr className="bg-slate-700/50">
              ...
              <th className="px-4 py-1 text-[10px] font-bold text-center">
                Ngày ghi
              </th>
              <th className="px-4 py-1 text-[10px] font-bold text-right border-l">
                Nợ
              </th>
          ...
          <tbody className="divide-y">
            {entries.map((e) => (
              <tr key={e.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="px-4 py-4 text-right tabular-nums bg-slate-50/30 border-l font-bold">
                  {e.debit > 0 ? formatVND(e.debit) : ""}
                </td>
                <td className="px-4 py-4 text-right tabular-nums bg-slate-50/30 font-bold">
            ...
            {entries.length === 0 && (
              <tr>
                <td
                  colSpan={7}
                  className="text-center py-10 italic text-muted-foreground opacity-50"
                >
