...
export default function GeneralLedgerPage() {
  ...
  const fetchLedger = useCallback(async () => {
    setLoading(true)
    try {
      const res = await axios.get(`/api/accounting/ledger/${accountId}?month=${month}&year=${year}&page=${page}&size=${pageSize}`, {
        headers
      })
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center bg-white p-4 rounded-xl shadow-sm border gap-4">
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-3 bg-slate-50/80 p-1.5 rounded-lg border shadow-sm">
            ...
            <div className="hidden lg:block h-6 w-[1px] bg-border"></div>
             ...
             <div className="flex items-center gap-2 px-2">
               <Input type="number" className="w-16 h-8 text-center bg-white font-medium shadow-sm transition-all focus:border-primary" value={month} onChange={e => {setPage(0); setMonth(Math.max(1, Math.min(12, Number(e.target.value))))}} />
               <span className="text-muted-foreground font-bold">/</span>
               <Input type="number" className="w-20 h-8 text-center bg-white font-medium shadow-sm transition-all focus:border-primary" value={year} onChange={e => {setPage(0); setYear(Math.max(2000, Number(e.target.value)))}} />
             </div>
