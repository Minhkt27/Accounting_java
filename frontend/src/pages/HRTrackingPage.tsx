...
export default function HRTrackingPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const resE = await axios.get(
          `/api/employees?size=2000&month=${mm}&year=${yyyy}`,
          auth,
        );
  ...
  const avgSalary =
    employees.length > 0
      ? employees.reduce((a, b) => a + (b.contractSalary || 0), 0) /
        employees.length
      : 0;
  ...
  const maternityCount = filteredLeaves.filter(
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <TrendingUp className="w-6 h-6 text-primary" /> Theo dõi Biến động &
          Nhân sự (HR Tracking)
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="p-6 bg-amber-50 border border-amber-200 rounded-2xl shadow-sm flex flex-col items-center">
          ...
          <p className="text-[10px] uppercase font-bold text-amber-700">
            Nhân viên nghỉ chế độ (Trong tháng)
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white border rounded-2xl shadow-md p-6">
          <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2 uppercase text-xs tracking-wider">
            <UserPlus className="w-4 h-4 text-blue-500" /> Danh sách Biến động
            chi tiết
          </h3>
          <div className="space-y-4">
            {employees
              ...
              .map((e) => (
                <div
                  key={e.id}
                  className="flex justify-between items-center p-3 hover:bg-slate-50 transition-colors rounded-xl border border-dashed"
                >
                  <div>
                  ...
                  <div className="text-right">
                    <p className="font-black text-xs mt-1 text-slate-600">
                      {formatVND(e.contractSalary || 0)}
                    </p>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-white border rounded-2xl shadow-md p-6">
          <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2 uppercase text-xs tracking-wider">
            <Calendar className="w-4 h-4 text-amber-500" /> Nhân viên nghỉ chế
            độ (Thai sản/Ốm đau)
          </h3>
          <div className="space-y-4">
            {filteredLeaves
              ...
              .map((l, i) => (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    >
                      {l.leaveType === "MATERNITY" ? "THAI SẢN" : "ỐM ĐAU"}
                    </span>
            ...
            {filteredLeaves.filter((l) => l.leaveType !== "ANNUAL").length ===
              ...
              <p className="text-center py-10 text-slate-400 italic text-sm">
                Chưa có bản ghi nghỉ chế độ nào.
              </p>
            )}
          </div>
        </div>
