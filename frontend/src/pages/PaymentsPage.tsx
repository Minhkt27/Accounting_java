    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center bg-white p-4 rounded-lg border border-gray-300 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 p-2 rounded-lg border border-gray-300 bg-white">
            <div className="flex items-center gap-2 border-r border-gray-300 pr-2 mr-2">
              <span className="text-sm font-normal text-black">Kỳ thanh toán</span>
              <Input type="number" className="w-16 h-8 text-center font-normal text-black border-gray-300" value={month} onChange={e => setMonth(Math.max(1, Math.min(12, Number(e.target.value))))} />
              <span className="text-black">/</span>
              <Input type="number" className="w-20 h-8 text-center font-normal text-black border-gray-300" value={year} onChange={e => setYear(Math.max(2000, Number(e.target.value)))} />
            </div>
            <div className="flex items-center gap-2">
              <Button size="default" variant="outline" onClick={handleExportExcel} className="h-8 gap-2 border-gray-300 text-black hover:bg-gray-50">
                  <FileSpreadsheet className="w-4 h-4" /> Excel
              </Button>
              <Button size="default" variant="outline" onClick={fetchData} className="h-8 font-normal border-gray-300 text-black hover:bg-gray-50">Tải lại</Button>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
            ...
            <div className={`p-5 rounded-lg border flex flex-col items-start text-left space-y-2 relative overflow-hidden transition-all ${
              paidSalary ? 'bg-gray-50 border-gray-200 opacity-70' : 'bg-white border-gray-300'
            }`}>
               {paidSalary && (
               ...
               <div>
                   <h3 className={`font-bold text-sm ${paidSalary ? 'text-gray-400' : 'text-black'}`}>Lương thực lĩnh (NET)</h3>
                   <p className={`text-xl font-bold mt-1 ${paidSalary ? 'text-gray-400 line-through' : 'text-black'}`}>{formatVND(totalNetPay)}</p>
            ...
            {/* Card 2: Nộp Bảo hiểm */}
            <div className={`p-5 rounded-lg border flex flex-col items-start text-left space-y-2 relative overflow-hidden transition-all ${
              paidInsurance ? 'bg-gray-50 border-gray-200 opacity-70' : 'bg-white border-gray-300'
            }`}>
               {paidInsurance && (
                 <div className="absolute top-3 right-3 flex items-center gap-1 text-black text-xs font-bold">
               ...
               <div>
                   <h3 className={`font-bold text-sm ${paidInsurance ? 'text-gray-400' : 'text-black'}`}>Bảo hiểm (NLĐ + DN)</h3>
                   <p className={`text-xl font-bold mt-1 ${paidInsurance ? 'text-gray-400 line-through' : 'text-black'}`}>{formatVND(totalInsurance)}</p>
            ...
            <div className={`p-5 rounded-lg border flex flex-col items-start text-left space-y-2 relative overflow-hidden transition-all ${
              paidTax ? 'bg-gray-50 border-gray-200 opacity-70' : 'bg-white border-gray-300'
            }`}>
               {paidTax && (
                 <div className="absolute top-3 right-3 flex items-center gap-1 text-black text-xs font-bold">
                   <CheckCircle2 className="w-4 h-4" /> Đã nộp
               ...
               <div>
                   <h3 className={`font-bold text-sm ${paidTax ? 'text-gray-400' : 'text-black'}`}>Thuế TNCN</h3>
                   <p className={`text-xl font-bold mt-1 ${paidTax ? 'text-gray-400 line-through' : 'text-black'}`}>{formatVND(totalTax)}</p>
                   <p className="text-xs text-gray-500 mt-1 font-bold">Nợ 3335 / Có 111, 112</p>
               </div>
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-gray-100 border-b border-gray-300">
                            <tr>
                                ...
                                <th className="px-4 py-3 font-bold text-black text-right">Số tiền</th>
                                <th className="px-4 py-3 font-bold text-black text-center">Trạng thái</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {payments.map((p) => (
                                <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="px-4 py-3 font-normal text-black tabular-nums">{p.voucherDate}</td>
                                    <td className="px-4 py-3">
                                        <div className="font-normal text-black">{p.voucherNumber || `PMT-${p.id}`}</div>
                                        <div className="text-xs text-gray-500">{p.type === 'UNC' ? 'UNC' : 'Phiếu chi'}</div>
                                    </td>
                                    <td className="px-4 py-3 font-normal text-black">
                                        {p.type === 'PHIEU_CHI' ? 'Tiền mặt' : 'Chuyển khoản'}
                                    </td>
                                    <td className="px-4 py-3 font-normal text-black text-xs">{p.description}</td>
                                    <td className="px-4 py-3 text-right font-normal text-black tabular-nums">
                                        {formatVND(p.totalAmount)}
                                    </td>
                                    <td className="px-4 py-3 text-center">
                                        <span className="text-xs font-normal text-black">Hoàn tất</span>
                                    </td>
                                </tr>
                            ))}
                            {payments.length === 0 && (
