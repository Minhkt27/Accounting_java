...
const DEFAULT_5_TAX_TIERS: TaxTier[] = [
  ...
  },
  {
    lowerBound: 10000000,
    upperBound: 30000000,
    lowerBoundYearly: 120000000,
    upperBoundYearly: 360000000,
    taxRate: 10,
    tierLevel: 2,
    status: "PENDING",
  },
  {
    lowerBound: 30000000,
    upperBound: 60000000,
    lowerBoundYearly: 360000000,
    upperBoundYearly: 720000000,
    taxRate: 20,
    tierLevel: 3,
    status: "PENDING",
  },
  {
    ...
    lowerBoundYearly: 720000000,
    upperBoundYearly: 1200000000,
    taxRate: 30,
    tierLevel: 4,
    status: "PENDING",
  },
  {
    ...
    lowerBoundYearly: 1200000000,
    upperBoundYearly: 999999999999,
    taxRate: 35,
    tierLevel: 5,
    status: "PENDING",
  },
...
export default function SalaryConfigPage() {
  const [activeTab, setActiveTab] = useState<"params" | "insurance" | "tax">(
    "params",
  );
  ...
  const [originalDeductions, setOriginalDeductions] =
    useState<DeductionSetting | null>(null);
  const [originalTaxTiers, setOriginalTaxTiers] = useState<TaxTier[] | null>(
    null,
  );
  const [originalInsuranceConfig, setOriginalInsuranceConfig] =
    useState<InsuranceConfig | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const {
        ...
        taxTiers: resTax,
        deductions: resDed,
        taxRules: resTaxRules,
      } = res.data;

      const p =
        resParams.find((x: SalaryParameter) => x.status === "PENDING") ||
        resParams.find((x: SalaryParameter) => x.status === "APPROVED");
      if (p) {
        setParams({
          ...p,
      ...
      const currentInsurance =
        resInsurance.find((x: InsuranceConfig) => x.status === "PENDING") ||
        resInsurance.find((x: InsuranceConfig) => x.status === "APPROVED");
      if (currentInsurance) {
      ...
      if (allTax.length === 0) {
        ...
        const hasPendingTax = allTax.some(
          (x: TaxTier) => x.status === "PENDING",
        );
      ...
      const d =
        resDed.find((x: DeductionSetting) => x.status === "PENDING") ||
        resDed.find((x: DeductionSetting) => x.status === "APPROVED");
      if (d) {
        setDeductions(d);
      } else {
        setDeductions({
  ...
  const saveParams = async () => {
    try {
      ...
      if (res.data.status === "PENDING") {
        showMsg(
          "Đã gửi đề xuất thay đổi tham số. Chờ Kế toán trưởng phê duyệt.",
          "info",
        );
      } else {
  ...
  const saveInsuranceConfig = async () => {
    try {
      const res = await axios.post(
        "/api/config/insurance-config",
        insuranceConfig,
        { headers },
      );
      if (res.data.status === "PENDING") {
        showMsg(
          "Đã gửi đề xuất thay đổi tỷ lệ bảo hiểm. Chờ phê duyệt.",
          "info",
        );
      } else {
  ...
  const approveInsuranceConfig = async (id?: number) => {
    ...
    try {
      await axios.post(
        `/api/config/insurance-config/${id}/approve`,
        {},
        { headers },
      );
  ...
  const rejectInsuranceConfig = async (id?: number) => {
    ...
    try {
      await axios.post(
        `/api/config/insurance-config/${id}/reject`,
        {},
        { headers },
      );
  ...
  const saveDeductions = async () => {
    try {
      const res = await axios.post("/api/config/deductions", deductions, {
        headers,
      });
      if (res.data.status === "PENDING") {
  ...
  const savePitTiers = async () => {
    try {
      ...
      const sortedTiers = [...taxTiers].sort(
  ...
  const approveTax = async () => {
    try {
      await Promise.all([
        ...
        axios.post(
          `/api/config/deductions/${deductions.id}/approve`,
          {},
          { headers },
        ),
      ]);
  ...
  const rejectTax = async () => {
    try {
      await Promise.all([
        ...
        axios.post(
          `/api/config/deductions/${deductions.id}/reject`,
          {},
          { headers },
        ),
      ]);
  ...
  const saveTaxRules = async () => {
    try {
      const res = await axios.post("/api/config/tax-rules", taxRules, {
        headers,
      });
      if (res.data[0]?.status === "PENDING") {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground font-medium text-sm">
            Thiết lập các tham số lương, bảo hiểm và biểu thuế TNCN
          </p>
        </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {params.status === "PENDING" ? (
                        <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] font-bold uppercase tracking-widest border border-amber-200">
                          Chờ duyệt
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase tracking-widest border border-emerald-200">
                          Đang áp dụng
                        </span>
                      )}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setParams({
                          ...params,
                          standardWorkDays: Math.max(0, Number(e.target.value)),
                        });
                      }}
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setParams({
                          ...params,
                          mealAllowance: Math.max(0, Number(raw)),
                        });
                      }}
                    />
                  </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-1">
                          Tỷ lệ đóng BHXH, BHYT, BHTN & KPCĐ
                        </p>
                      </div>
                    </div>
                        ...
                        {[
                          ...
                          <div key={item.key} className="space-y-2">
                            ...
                            <div className="relative">
                              ...
                              />
                              <span className="absolute right-4 top-1/2 -translate-y-1/2 text-lg font-normal text-black">
                                %
                              </span>
                            </div>
                          </div>
                    ...
                    {isInsuranceEditing ? (
                      ...
                      (userRoles.includes("ROLE_KE_TOAN_TRUONG") ||
                        ...
                        <div className="flex gap-4">
                          <Button
                            onClick={() =>
                              approveInsuranceConfig(insuranceConfig.id)
                            }
                            className="gap-2 h-14 px-10 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-xl shadow-emerald-200 font-bold text-sm transition-all hover:scale-105 active:scale-95"
                          >
                          ...
                          <Button
                            onClick={() =>
                              rejectInsuranceConfig(insuranceConfig.id)
                            }
                            variant="outline"
                            className="gap-2 h-14 px-10 rounded-2xl border-red-200 text-red-600 hover:bg-red-50 font-bold text-sm transition-all hover:scale-105 active:scale-95"
                          >
              ...
              <div className="space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            setDeductions({
                              ...deductions,
                              personalDeduction: Math.max(0, Number(raw)),
                            });
                          }}
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            setDeductions({
                              ...deductions,
                              dependentDeduction: Math.max(0, Number(raw)),
                            });
                          }}
                        />
                      </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          (userRoles.includes("ROLE_KE_TOAN_TRUONG") ||
                            ...
                            <div className="flex flex-col w-full gap-2">
                              <Button
                                onClick={approveTax}
                                className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase shadow-lg shadow-emerald-200"
                              >
                                PHÊ DUYỆT
                              </Button>
                              <Button
                                onClick={rejectTax}
                                variant="outline"
                                className="w-full h-12 rounded-xl border-red-200 text-red-600 hover:bg-red-50 font-bold text-xs uppercase"
                              >
                                TỪ CHỐI
                              </Button>
                            </div>
                            ...
                            <>
                              ...
                              <Button
                                ...
                                className="flex-1 h-12 rounded-xl bg-[#111827] text-white font-bold text-xs uppercase shadow-lg shadow-slate-200"
                              >
                                {userRoles.includes("ROLE_ADMIN")
                                  ? "LƯU NGAY"
                                  : "GỬI ĐỀ XUẤT"}
                  ...
                  <div className="lg:col-span-2 space-y-6">
                            ...
                            onClick={() =>
                              setTaxTiers([
                                ...taxTiers,
                                {
                                  ...
                                  upperBoundYearly: 0,
                                  taxRate: 0,
                                  tierLevel: taxTiers.length + 1,
                                  status: "PENDING",
                                },
                              ])
                            }
                            className="rounded-xl font-bold text-[10px] uppercase gap-2 h-9 border-slate-200 hover:bg-slate-50"
                          >
                    ...
                    <div className="border border-slate-400 shadow-sm overflow-hidden bg-white">
                      {/* ==============================================
                                        KHU VỰC VẼ BẢNG: CẤU HÌNH THAM SỐ GIẢM TRỪ THUẾ
                                        ============================================== */}
                      <table className="w-full text-sm border-collapse">
                        <thead>
                          <tr className="border-b border-slate-400">
                            <th className="px-4 py-4 font-semibold text-slate-800 text-center w-24 border-r border-slate-400 bg-slate-50">
                              Bậc thuế
                            </th>
                            <th className="px-6 py-4 font-semibold text-slate-800 text-center border-r border-slate-400 bg-slate-50 leading-relaxed">
                              ...
                              <br />
                              <span className="text-sm font-normal text-slate-600 tracking-tight">
                                (triệu đồng)
                              </span>
                            </th>
                            <th className="px-6 py-4 font-semibold text-slate-800 text-center border-r border-slate-400 bg-slate-50 leading-relaxed">
                        ...
                        <tbody className="divide-y divide-slate-400">
                          {[...taxTiers]
                            ...
                            .map((t) => {
                              ...
                              const minM_year =
                                (t.lowerBoundYearly || 0) / 1000000;
                              const maxM_year =
                                (t.upperBoundYearly || 0) / 1000000;
                              ...
                              return (
                                <tr
                                  key={t.tierLevel}
                                  className="hover:bg-slate-50/50 transition-colors text-slate-900"
                                >
                                  <td className="px-4 py-4 text-center font-medium border-r border-slate-400">
                                    {t.tierLevel}
                                  </td>
                                        ...
                                        <Input
                                          ...
                                          onChange={(e) => {
                                            ...
                                            const n = taxTiers.map((tier) => {
                                              if (
                                                tier.tierLevel === t.tierLevel
                                              )
                                                return {
                                                  ...tier,
                                                  lowerBoundYearly: val,
                                                  lowerBound: monthlyVal,
                                                };
                                              if (
                                                tier.tierLevel ===
                                                t.tierLevel - 1
                                                ...
                                                return {
                                                  ...tier,
                                                  upperBoundYearly: val,
                                                  upperBound: monthlyVal,
                                                };
                                              return tier;
                                            });
                                            setTaxTiers(n);
                    ...
                    <div className="flex items-center justify-end pt-4 gap-3">
                      {!isPitEditing ? (
                        ...
                        <>
                          <Button
                            variant="outline"
                            onClick={cancelPitEdit}
                            className="h-12 px-8 rounded-xl font-bold text-xs uppercase border-slate-200"
                          >
                            HỦY BỎ
                          </Button>
                          <Button
                            onClick={savePitTiers}
                            className="h-12 px-10 rounded-xl bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs uppercase shadow-lg shadow-orange-500/20"
                          >
                ...
                <div className="space-y-8 pt-10 border-t border-slate-100">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-slate-800 uppercase italic underline decoration-emerald-200 decoration-4 underline-offset-8">
                        Thuế TNCN
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="border border-slate-100 rounded-3xl overflow-hidden shadow-sm">
                    {/* ==============================================
                                    KHU VỰC VẼ BẢNG: CẤU HÌNH THANG BẬC THUẾ LŨY TIẾN
                                    ============================================== */}
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                          <th className="px-8 py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400">
                            Loại nhân sự
                          </th>
                          <th className="px-6 py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400 text-center">
                            Miễn thuế
                          </th>
                          <th className="px-6 py-5 font-bold uppercase text-[10px] tracking-widest text-slate-400 text-center">
                            10% Cố định
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {[
                          ...
                          const config = taxRules.find(
                            (r) => r.employeeType === row.type,
                          ) || {
                            employeeType:
                              row.type as EmployeeTaxConfig["employeeType"],
                            taxMethod: "PROGRESSIVE",
                          };
                          const updateMethod = (
                            method: "EXEMPT" | "FIXED_10" | "PROGRESSIVE",
                          ) => {
                            ...
                            >
                                ...
                                <label className="inline-flex items-center cursor-pointer group">
                                  ...
                                  >
                                    <input
                                      type="radio"
                                      name={`tax_${row.type}`}
                                      checked={config.taxMethod === "EXEMPT"}
                                      onChange={() => updateMethod("EXEMPT")}
                                      className="hidden"
                                    />
                                    {config.taxMethod === "EXEMPT" && (
                                  ...
                                  <span
                                    className={`ml-3 text-xs font-bold transition-colors ${config.taxMethod === "EXEMPT" ? "text-slate-900" : "text-slate-400"}`}
                                  >
                                    Miễn thuế
                                  </span>
                                </label>
                                ...
                                <label className="inline-flex items-center cursor-pointer group">
                                  ...
                                  >
                                    <input
                                      type="radio"
                                      name={`tax_${row.type}`}
                                      checked={config.taxMethod === "FIXED_10"}
                                      onChange={() => updateMethod("FIXED_10")}
                                      className="hidden"
                                    />
                                    {config.taxMethod === "FIXED_10" && (
                                      <div className="w-1.5 h-1.5 rounded-full bg-white shadow-sm" />
                                  ...
                                  <span
                                    className={`ml-3 text-xs font-bold transition-colors ${config.taxMethod === "FIXED_10" ? "text-slate-900" : "text-slate-400"}`}
                                  >
                                    10%
                                  </span>
                                </label>
                              ...
                              <td className="px-6 py-5 text-center">
                                <label className="inline-flex items-center cursor-pointer group">
                                  ...
                                  <span
                                    className={`ml-3 text-xs font-bold transition-colors ${config.taxMethod === "PROGRESSIVE" ? "text-slate-900" : "text-slate-400"}`}
                                  >
                                    Theo biểu lũy tiến
                                  </span>
                                </label>
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="pt-2 flex items-center gap-4">
                      {taxRules.some((r) => r.status === "PENDING") &&
                      (userRoles.includes("ROLE_KE_TOAN_TRUONG") ||
                        userRoles.includes("ROLE_ADMIN")) ? (
                        <Button
                          onClick={approveTaxRules}
                          className="gap-2 h-14 px-10 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white shadow-xl shadow-emerald-200 font-bold text-sm"
                        >
                          <CheckCircle2 size={18} /> PHÊ DUYỆT PHƯƠNG THỨC TÍNH
                          THUẾ
                        </Button>
                      ) : (
                        <Button
                        ...
                        >
                          <Save size={18} />{" "}
                          {userRoles.includes("ROLE_ADMIN")
                            ? "LƯU CẤU HÌNH THUẾ NGAY"
                            : "GỬI ĐỀ XUẤT CẤU HÌNH THUẾ"}
                        </Button>
                      )}
                    </div>
                  </div>
