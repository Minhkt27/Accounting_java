...
export default function UserManagementPage() {
  ...
  const isAllowed = (roleName: string, functionCode: string) => {
    ...
    const p = perms.find(
      (x) => x.roleName === roleName && x.functionCode === functionCode,
    );
    ...
    try {
      if (editingUser) {
        await axios.put(
          ...
          { email: formEmail, roles: formRoles },
          { headers },
        );
      } else {
        await axios.post(
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground font-medium text-sm">
            Quản lý tài khoản người dùng và phân quyền chức năng
          </p>
        </div>
        <Button
          onClick={openCreate}
      ...
      <div className="grid grid-cols-1 gap-10">
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900 uppercase tracking-tight italic">
                Ma trận <span className="text-primary italic">Phân quyền</span>
              </h2>
            </div>
            <Button
              onClick={savePerms}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
            <div className="overflow-x-auto">
              {/* ==============================================
                          KHU VỰC VẼ BẢNG: QUẢN LÝ TÀI KHOẢN NGƯỜI DÙNG
                          ============================================== */}
              <table className="w-full text-left">
                <thead className="bg-primary text-primary-foreground">
                  <tr className="border-b border-white/10">
                    <th className="px-4 py-4 text-[9px] font-bold uppercase text-white/70 tracking-widest pl-8">
                      Chức năng hệ thống
                    </th>
                    <th className="px-4 py-4 text-[9px] font-bold uppercase text-white/70 tracking-widest text-center">
                      Admin
                    </th>
                    {EDITABLE_ROLES.map((r) => (
                      <th
                        key={r}
                        className="px-4 py-4 text-[9px] font-bold uppercase text-white/70 tracking-widest text-center"
                      >
                        {ROLE_LABELS[r]}
                      </th>
                    ))}
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tight italic">
                Danh sách <span className="text-primary italic">Tài khoản</span>
              </h2>
            </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
            <div className="overflow-x-auto">
              {/* ==============================================
                          KHU VỰC VẼ BẢNG: QUẢN LÝ VAI TRÒ
                          ============================================== */}
              <table className="w-full text-left">
                <thead className="bg-primary text-primary-foreground">
                  <tr className="border-b border-white/10">
                    <th className="px-8 py-5 text-[10px] font-bold uppercase text-white/70 tracking-widest pl-10">
                      Người dùng
                    </th>
                    <th className="px-8 py-5 text-[10px] font-bold uppercase text-white/70 tracking-widest">
                      Email liên hệ
                    </th>
                    <th className="px-8 py-5 text-[10px] font-bold uppercase text-white/70 tracking-widest">
                      PF / Quyền hạn
                        ...
                        <td className="px-8 py-6">
                          <div className="flex flex-wrap gap-2">
                            {u.roles.map((r) => (
                              <span
                                key={r}
                                className="px-3 py-1 rounded-full bg-slate-50 text-[9px] font-bold uppercase tracking-widest text-slate-500 border border-slate-200"
                              >
                                {ROLE_LABELS[r]}
                              </span>
                            ))}
                        ...
                        <td className="px-8 py-6 text-center pr-10">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              ...
                              className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:bg-amber-50 hover:text-amber-500 transition-all"
                              title="Đổi mật khẩu"
                            >
                              <Key size={16} />
                            </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
            ...
            >
              <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 uppercase tracking-tighter italic leading-none">
                    {editingUser ? "Cập nhật" : "Tạo mới"}{" "}
                    <span className="text-primary italic">Người dùng</span>
                  </h3>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-2">
                    Phân vai trò <span className="text-red-500">*</span>
                  </label>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {roles
                      .filter((r) => r.name !== "ROLE_ADMIN")
                      .map((r) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${selected ? "bg-primary border-primary" : "border-slate-200"}`}
                            >
                              {selected && (
                                <CheckCircle2
                                  size={12}
              ...
              <div className="flex gap-4 pt-4 border-t border-slate-100 mt-4">
                <Button
                  onClick={handleSave}
                  className="flex-1 h-14 rounded-2xl bg-primary hover:bg-indigo-700 text-white font-bold text-xs uppercase tracking-widest shadow-xl shadow-indigo-500/20 active:scale-95"
                >
                ...
                <Button
                  ...
                  className="px-8 h-14 rounded-2xl font-bold text-xs uppercase text-slate-400 hover:bg-slate-50"
                >
                  Đóng
                </Button>
              </div>
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-start gap-3">
                  ...
                  <p className="text-[9px] text-blue-700 font-bold uppercase leading-relaxed text-left">
                    Mật khẩu mới sẽ có hiệu lực ngay lập tức sau khi xác nhận.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 pt-4">
                <Button
                  ...
                  className="h-14 rounded-2xl bg-[#111827] hover:bg-black text-white font-bold text-xs uppercase tracking-widest shadow-xl shadow-slate-200 active:scale-95"
                >
                  Xác nhận đổi
                </Button>
                <Button
                  ...
                  className="h-14 rounded-2xl font-bold text-xs uppercase text-slate-400 hover:bg-slate-50"
                >
                  Hủy bỏ
                </Button>
              </div>
