      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 p-2 rounded-lg border border-gray-300 bg-white">
          <div className="flex items-center gap-2 border-r border-gray-300 pr-3">
            ...
            <Input type="number" className="w-16 h-8 text-center font-normal text-black border-gray-300" value={month} onChange={e => handleMonthChange(Math.max(1, Math.min(12, Number(e.target.value))))} />
            <span className="text-black">/</span>
            <Input type="number" className="w-20 h-8 text-center font-normal text-black border-gray-300" value={year} onChange={e => handleYearChange(Math.max(2000, Number(e.target.value)))} />
          </div>
          <Button variant="outline" onClick={handleExportExcel} className="gap-2 h-8 border-gray-300 text-black hover:bg-gray-50">
      ...
      <div className="flex gap-1 p-1 rounded-lg border border-gray-300 bg-white w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-normal transition-all ${
            tab === "vouchers" ? "bg-gray-800 text-white" : "text-black hover:bg-gray-100"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-normal transition-all ${
            tab === "journal" ? "bg-gray-800 text-white" : "text-black hover:bg-gray-100"
          }`}
        >
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-100 border-b border-gray-300">
                <tr>
                  ...
                  <th className="px-4 py-3 font-bold text-black text-right">Tổng tiền</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {vouchers.map(v => (
                  <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-normal text-black">{v.voucherNumber}</td>
                    <td className="px-4 py-3 font-normal text-black">
                      {v.type === 'PHIEU_CHI' ? 'Phiếu chi' : 'Ủy nhiệm chi'}
                    </td>
                    <td className="px-4 py-3 font-normal text-black">{v.voucherDate}</td>
                    <td className="px-4 py-3 text-right font-normal text-black tabular-nums">{formatVND(v.totalAmount)}</td>
                    <td className="px-4 py-3 font-normal text-black">{v.description}</td>
                  </tr>
                ))}
                {vouchers.length === 0 && (
                  <tr>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-100 border-b border-gray-300">
                <tr>
                  ...
                  <th className="px-4 py-3 font-bold text-black text-center">TK Nợ</th>
                  <th className="px-4 py-3 font-bold text-black text-center">TK Có</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {journal.map(j => (
                  <tr key={j.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3 font-normal text-black tabular-nums">{j.voucherDate}</td>
                    <td className="px-4 py-3 font-normal text-black">{j.voucherNumber}</td>
                    <td className="px-4 py-3 text-center font-normal text-black">
                      <span>Nợ {j.debitAccountId}</span>
                      <p className="text-xs text-gray-500 mt-0.5">{j.debitAccountName}</p>
                    </td>
                    <td className="px-4 py-3 text-center font-normal text-black">
                      <span>Có {j.creditAccountId}</span>
                      <p className="text-xs text-gray-500 mt-0.5">{j.creditAccountName}</p>
                    </td>
                    <td className="px-4 py-3 text-right font-normal text-black tabular-nums">{formatVND(j.amount)}</td>
                    <td className="px-4 py-3 font-normal text-black text-xs">{j.description}</td>
                  </tr>
                ))}
                {journal.length === 0 && (
                  <tr>
              ...
              {journal.length > 0 && (
                <tfoot className="bg-gray-50 border-t border-gray-300">
                  <tr>
                    <td colSpan={4} className="px-4 py-3 text-right font-bold text-black text-sm">Tổng cộng phát sinh:</td>
                    <td className="px-4 py-3 text-right font-bold text-black tabular-nums">{formatVND(totalJournalDebit)}</td>
                    <td></td>
                  </tr>
                </tfoot>
              )}
