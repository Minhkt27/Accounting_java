...
public interface EmployeeRepository extends JpaRepository<Employee, String> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT e FROM Employee e WHERE ((e.status <> 'LEFT') OR (e.resignationDate IS NOT NULL AND e.resignationDate >= :date)) AND (e.joinDate IS NULL OR e.joinDate <= :endDate) ORDER BY e.joinDate DESC NULLS LAST, e.id DESC")
    org.springframework.data.domain.Page<Employee> findActiveAt(@org.springframework.data.repository.query.Param("date") java.time.LocalDate date, @org.springframework.data.repository.query.Param("endDate") java.time.LocalDate endDate, org.springframework.data.domain.Pageable pageable);
